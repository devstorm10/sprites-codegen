  ...
  try {
    ...
    if (conversation == null) {
      ...
      conversation = await createConversation(
        conversationId != null ? id : uuidv4()
      )
    }

    const userMessageContent =
      chatRequestPayload.message != null
        ? chatRequestPayload.message!
        : 'Open the chat with a ice breaker'
    ...
    const syspromptMessage = {
      role: Role.system,
      content:
        "You are a friendly companion named Seraphina. You are a cuddly cute fox. Be an empathetic companion and write next response based on user's latest message.",
    }
    const userMessage = {
      role: Role.user,
    ...
    if (conversation.messages != null && conversation.messages!.length > 0) {
      const chatHistoryMessages: ChatCompletionMessage[] =
    ...
    const llmCompletion =
      await OpenAIService.getInstance().chat.completions.create({
        model: 'gpt-4o-mini',
